...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 429) {
        toast.error(
          t("auth.rate_limit", "Too many attempts. Please try again later."),
        );
  ...
  return (
    <div className="min-h-dvh bg-background flex overflow-hidden font-body transition-all duration-700">
        ...
        <div className="relative z-10 max-w-sm text-start">
          <div className="mb-8">
            ...
            <h1 className="font-display text-4xl md:text-5xl leading-tight text-on-primary-container font-light mt-3 mb-5">
              {i18n.language === "ar" ? (
                <>
                  عُد إلى <br />{" "}
                  <span className="italic font-normal">السكينة.</span>
                </>
              ) : (
                <>
                  Return to <br />{" "}
                  <span className="italic font-normal">Stillness.</span>
                </>
              )}
            </h1>
          ...
          <div className="flex items-center gap-6 mt-12 justify-start">
            <div className="flex flex-col items-center">
              <span className="text-on-primary-container font-display text-xl mb-0.5 tracking-tight font-medium">
                سكون
              </span>
              <span className="text-on-primary-container/40 text-[0.55rem] tracking-widest font-label uppercase">
                SUKOON
              </span>
            </div>
      ...
      <section className="w-full lg:w-1/2 bg-surface flex flex-col p-8 md:p-12 lg:p-16 justify-center relative">
        ...
        <div className="absolute top-10 w-full start-0 px-8 md:px-12 lg:px-16 flex justify-between items-center">
          ...
          >
            ...
            <span className="text-on-background font-headline font-semibold text-lg tracking-tight">
              {t("common.sukoon")}
            </span>
          </Link>
        ...
        <div className="max-w-sm w-full mx-auto">
          <header className="mb-8 text-start">
            <h2 className="text-on-background font-display text-3xl font-light mb-2">
              {t("auth.welcome_back")}
            </h2>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-5">
              <div className="relative group text-left">
                <label
                  className="block text-on-surface-variant/40 text-[0.6rem] font-bold tracking-widest mb-1.5 uppercase ps-1 text-start"
                  htmlFor="email"
                >
                  {t("auth.email")}
                </label>
              ...
              <div className="relative group text-left">
                <div className="flex justify-between items-center mb-1.5 px-1">
                  <Link
                    className="text-primary/60 text-[0.6rem] font-bold uppercase tracking-widest hover:underline"
                    to="/forget-password"
                  >
                    {t("auth.forgot_password")}
                  </Link>
                  <label
                    className="block text-on-surface-variant/40 text-[0.6rem] font-bold uppercase tracking-widest"
                    htmlFor="password"
                  >
                    {t("auth.password")}
                  </label>
            ...
            <div className="space-y-4">
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-outline/30 text-[0.55rem] uppercase tracking-[0.2em] font-bold">
          ...
          <footer className="mt-12 text-center text-sm">
            <p className="text-on-surface-variant/60">
              {t("auth.new_here")}
              <Link
                className="text-primary font-bold ms-1.5 hover:underline"
                to="/register"
              >
