...
class PersonalizedAyaService {
  constructor(private personalizedAya: PersonalizedAyaRepo) {}

  async getPersonalizedAya(userId: number) {
    return await this.personalizedAya.getPersonalizedAya(userId);
  }

  async personalizedAyaWithAI(userId: number) {
    const allTafreeghs = await new TafreeghRepository().recentTafreeghs(userId);

    if (allTafreeghs.length === 0) return;

    const tafreeghEntries = allTafreeghs.map(
      (t: (typeof allTafreeghs)[number]) => ({
    ...
    if (tafreeghEntries.length === 0) return;
    ...
    if (validated) {
      ...
      return this.personalizedAya.createPersonalizedAya(
        userId,
        { ...ayaFromApi },
        validated.reason,
      );
