...
import {
  ...
  ResetPasswordSchema,
  ChangeFirstNameAndLastNameSchema,
...
authRouter.patch(
  ...
  tryCatch(authController.uploadAvatar),
);
...
authRouter.put(
  "/change-name",
  checkAuth,
  validation(ChangeFirstNameAndLastNameSchema),
  tryCatch(authController.changeFirstNameAndLastName),
);
...
authRouter.delete(
  "/delete-account",
  checkAuth,
  tryCatch(authController.deleteAccount),
);
