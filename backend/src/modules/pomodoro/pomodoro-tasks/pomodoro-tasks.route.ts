...
import {
  createPomodoroTaskSchema,
  updatePomodoroTaskSchema,
} from "./pomodoro-tasks.schema.js";
...
const pomodoroTasksController = new PomodoroTasksController(
  pomodoroTaskService,
);
...
pomodoroTasksRouter.get(
  "/tasks/available",
  tryCatch(pomodoroTasksController.getAvailableTasks),
);
pomodoroTasksRouter.get(
  "/tasks/:id",
  tryCatch(pomodoroTasksController.getById),
);
pomodoroTasksRouter.post(
  "/tasks",
  validation(createPomodoroTaskSchema),
  tryCatch(pomodoroTasksController.create),
);
pomodoroTasksRouter.patch(
  "/tasks/:id",
  validation(updatePomodoroTaskSchema),
  tryCatch(pomodoroTasksController.update),
);
pomodoroTasksRouter.delete(
  "/tasks/:id",
  tryCatch(pomodoroTasksController.delete),
);

// Sessions routes
pomodoroTasksRouter.use("/session", pomodoroSessionsRouter);
