...
class TasksController {
    ...
    createUpcomingTask = async (req, res) => {
        const { title, description, listId, tagIds } = req.body;
        const userId = req.user.id;
    ...
    createTaskByList = async (req, res) => {
        const userId = req.user.id;
        const listId = Number(req.params.listId);
        if (isNaN(listId)) {
            throw AppError.BadRequest("Invalid list ID");
        ...
        const task = await this.taskService.createTask({ title, description, listId, tagIds, userId }, userId);
        res.status(201).json(task);
    };
    createTaskByTag = async (req, res) => {
        const userId = req.user.id;
        const tagId = Number(req.params.tagId);
        if (isNaN(tagId)) {
            throw AppError.BadRequest("Invalid tag ID");
        ...
        const task = await this.taskService.createTask({ title, description, listId, tagIds: updatedTagIds, userId }, userId);
        res.status(201).json(task);
    };
