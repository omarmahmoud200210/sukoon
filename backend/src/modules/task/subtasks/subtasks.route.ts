...
const subTaskService = new SubTaskService(subTaskRepository, taskRepository);
const subTasksController = new SubTasksController(subTaskService);

subTaskRouter.get("/:taskId", tryCatch(subTasksController.getAll));
...
subTaskRouter.delete(
  "/:taskId/:subTaskId",
  tryCatch(subTasksController.delete),
);
